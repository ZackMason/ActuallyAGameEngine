...
struct logger_t {
    ...
    inline static std::array<fmt::v8::text_style, (size_t)log_level_e::SIZE> log_color = {
    ...
    inline static std::array<std::string, (size_t)log_level_e::SIZE> level_to_string = {
        "Info",
        "Alloc",
        "Warning",
        "Error",
    ...
    static void log(const std::string& message, log_level_e l) {
        if (level <= l) {
            if (console) {
                fmt::print(log_color[(size_t)l], "{} - {}\n", level_to_string[(size_t)l], message);
            }
            if (!output.empty()) {
